...
class Lobby {
  ...
  updatePlayerReady(nickname, ready) {
    this.playerReadyStatus[nickname] = ready;
    const readyPlayers = Object.values(this.playerReadyStatus).filter(val => val === true).length;
    this.broadcast('gameStartUpdate', {
      readyPlayers: readyPlayers,
      totalPlayers: this.numPlayers,
      message: `Giocatori pronti: ${readyPlayers}/${this.numPlayers}`
    });
    if (readyPlayers === this.numPlayers) {
      // Resetta lo stato dei giocatori e avvia il gioco
      Object.keys(this.playerReadyStatus).forEach(key => {
        this.playerReadyStatus[key] = false;
      });
      this.broadcast('gameStarted', { message: 'Il gioco sta per iniziare!' });
      this.assignRooms();
    }
  }
  ...
  assignRooms() {
    ...
    const room2Players = [this.users[randomIndex].nickname];
    const room4Players = this.users.filter((u, i) => i !== randomIndex).map(u => u.nickname);
    this.roomAssignments = { room2: room2Players, room4: room4Players };
    this.broadcast('roomAssignment', this.roomAssignments);
  }
  ...
  verifyQuiz(area, code, callback) {
    ...
    if (quiz && quiz.checkSolution(code)) {
      ...
      if (Object.keys(this.verifiedQuizSolutions).length === 3) {
        this.broadcast('gameWon', { message: 'Congratulations. You won the game' });
      }
  ...
  initPuzzle() {
    ...
    this.broadcast("initGame", this.puzzle.boardState);
  }
}
...
class ServerManager {
  ...
  setupSocket() {
    this.io.on('connection', (socket) => {
      ...
      // Durante la creazione della lobby
...
socket.on('joinLobby', (data) => {
  ...
  if (lobby) {
    ...
    // Gestione del ready, usando socket.nickname e socket.lobbyCode
socket.on('playerReady', (data) => {
  ...
  if (lobbyCode && this.lobbies[lobbyCode]) {
    ...
    lobby.updatePlayerReady(nickname, ready);
  }
});
      ...
      socket.on('verifyQuiz', (data, callback) => {
        const lobby = this.lobbies[data.lobbyCode];
        if (lobby) {
          lobby.verifyQuiz(data.area, data.code, callback);
        } else {
      ...
      socket.on("tileMoved", (data) => {
        ...
        if (lobby && lobby.puzzle) {
          lobby.puzzle.boardState[data.oldGridY][data.oldGridX] = lobby.puzzle.boardState[data.newGridY][data.newGridX];
          lobby.puzzle.boardState[data.newGridY][data.newGridX] = data.tileNumber;
